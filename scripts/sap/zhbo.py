...
def main():
    ...
    if not args.filename: args.filename=f"zhbo_{datetime.now().strftime('%d-%m-%Y')}.xls"
    ensure_dir(args.output)
    print("INICIANDO ZHBO (HOMOLOGADO)\n"+"="*60)
    try:
        _,_,session,rc,rs=attach_to_sap(args.conn,args.sess)
        print(f"Conexion={rc}, Sesion={rs}\nRow: {args.row} | Fecha: {date_str}\nSalida: {args.output}\\{args.filename}\nDebug: {'ON' if args.debug else 'OFF'}\n"+"="*60)
        full_path=run_zhbo(session,args.row,args.output,args.filename,date_str,debug=args.debug)
        print("\n"+"="*60+"\nPROCESO ZHBO COMPLETADO\n"+"="*60)
